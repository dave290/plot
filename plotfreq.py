...
#plots intensity (K) vs frequency (MHz)

#enter lower and upper bounds for frequency
lowerbound=1419201171
upperbound=1421598828
...
GALLON=[0];GALLAT=[0];frequency=[0];intensity=[0];correction=0;colon=[999]
...
#read file info
GALLON=[0];GALLAT=[0];frequency=[0];intensity=[0]
with open(datafile,'r') as f:
    N=0
    for line in f:
        N=N+1
        ...
        if N>=startline and N<=endline:
            ...
            linestring=line.split()      
            x=(float(linestring[1]))/1000000.0
            frequency.append(x)
            y=float(linestring[2])
            intensity.append(y)
f.closed
True
GALLON.pop(0);GALLAT.pop(0);frequency.pop(0);intensity.pop(0)

#plot intensity vs frequency
fig=plt.figure(figsize=(10,6))
ax=plt.axes()
ax.set_title("Intensity vs Frequency",fontsize=20)
ax.set_xlabel("Frequency (MHz)", fontsize=18)
ax.set_ylabel("Intensity (Kelvins)", fontsize=18)
plt.xticks(fontsize=16)
plt.yticks(fontsize=16)
ax.scatter(frequency,intensity, c="red",s=60,label=("GLONG,GLAT= "+str(int(GALLON[0]))+"  "+str(int(GALLAT[0]))))
ax.legend(loc="best")
plt.show()
